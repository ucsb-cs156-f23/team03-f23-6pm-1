...
describe("ArticlesTable tests", () => {
  ...
  const expectedHeaders = ["id", "Title", "Url", "Explanation", "Date"];
  const expectedFields = ["id", "title", "url", "explanation", "dateAdded"];
  ...
  test("Has the expected column headers, content and buttons for admin user", () => {
    ...
    expect(screen.getByTestId(`${testId}-cell-row-0-col-title`)).toHaveTextContent("testtitle");

    expect(screen.getByTestId(`${testId}-cell-row-1-col-id`)).toHaveTextContent("2");
    expect(screen.getByTestId(`${testId}-cell-row-1-col-title`)).toHaveTextContent("testtitle2");
